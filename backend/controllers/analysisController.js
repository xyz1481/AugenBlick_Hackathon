...
const analyzeTopic = async (req, res) => {
  ...
  try {
    ...
    if (!indicatorData) {
      return res.status(500).json({ error: 'Market Data Source (Yahoo) is currently unreachable. Please try again or use a demo symbol like CL=F.' });
    }

    // 3. Combine Narratives and Analyze Sentiment
    const newsSources = (newsData && newsData.topNarratives) ? newsData.topNarratives : [];
    const redditSources = (redditData && redditData.posts) ? redditData.posts : [];
    const twitterSources = (twitterData && twitterData.tweets) ? twitterData.tweets : [];
    
    console.log(`[Analysis] Found ${newsSources.length} news, ${redditSources.length} reddit, ${twitterSources.length} tweets for "${topic}"`);
    ...
    const combinedText = `${newsText} ${redditText} ${twitterText}`.trim();
    ...
    if (!hasNarrativeData) {
      ...
      conclusion = "Insufficient public narrative data found for a definitive AI verdict. " + 
                  (indicatorPanic ? "However, market data shows high volatility." : "Market data appears stable.");
    } else if (narrativePanic && !indicatorPanic) {
      panicIndex = 85;
      conclusion = "Narrative warning: Social signals show significant anxiety/panic, but real-world market indicators remain stable. High probability of misinformation or overreaction.";
    } else if (narrativePanic && indicatorPanic) {
      panicIndex = 45;
      conclusion = "Systemic Risk: Public anxiety aligns with measurable market volatility. The narrative is backed by hard data.";
    } else if (!narrativePanic && indicatorPanic) {
      panicIndex = 25;
      conclusion = "Data Alert: Markets are showing volatility that is not yet reflected in public discourse. Possible brewing crisis.";
    } else {
      panicIndex = 10;
      conclusion = "Equilibrium: Both public narrative and market signals are currently within normal stability ranges.";
    }
    ...
    const analysisResult = new Analysis({
      ...
      narrative: {
        ...
        newsSources: newsSources.map(n => ({
          title: n.title,
          link: n.link,
          source: (n.source && typeof n.source === 'object') ? n.source.title : (n.source || 'News'),
    ...
    console.log(`[Analysis] Saved verdict for "${topic}". Panic Index: ${panicIndex}%`);
